# Bonus feature #3 - class for each move
...
class Move
  ...
  CLASS_NAMES = { "rock" => Rock,
                  "paper" => Paper,
                  "scissors" => Scissors,
                  "lizard" => Lizard,
                  "spock" => Spock }

  LEGAL_PLAYS = CLASS_NAMES.keys
  ...
  def winner?(other_move)
    CLASS_NAMES[value]::BEATS.include?(other_move.value) ? 1 : - 1
  end
end
...
# Before coding:
#   This does not feel like it will be a good decision.
#   The only thing we do with the moves is to assign them to players,
#   and then compare them. They don't have much in the way of behavior/attributes to 
#   justify separate classes.

# Design decision - 
#     1) keep Move class, and have individual move classes
#        inherit from it. This seems silly
#     2) 

# After coding
# This was pointless - it created no additional benefit,
# while making the code a lot messier and harder to maintain
