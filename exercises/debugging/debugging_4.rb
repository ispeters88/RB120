...
# > We have defined `==` as a private method within our `Employee` class.
# However, we attempt to call this method from `EmployeeManagementSystem` each time we check if
# an employee already exists in our list of employees. We need to provide further access to this method
# We can do so by changing the `private` keyword to `protected` - this works because, even though
# we are calling the method from within `EmployeeMaanagementSystem`, we are invoking it on an
# `Employee` object.