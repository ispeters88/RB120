...
class TaskManager
  ...
  def display_high_priority_tasks
    high_pri_tasks = tasks.select do |task|
      task.priority == :high
    end

    display(high_pri_tasks)
  end
...
# > The problem with this one is we did not specify whether `tasks` in the display_high_priority_tasks method
# is a local variable or a class instance variable. If we don't specify this, then Ruby will assume it is a local variable
# We haven't assigned a value to such a a local, so it is equal to `nil`, and we can't use #select on `nil``
